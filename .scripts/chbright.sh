...
#h SUBSYSTEM=="backlight",RUN+="/bin/chgrp netdev /sys/class/backlight/%k/brightness /sys/class/backlight/%k/bl_power",RUN+="/bin/chmod 664 /sys/class/backlight/%k/brightness /sys/class/backlight/%k/bl_power"
#h ---end---
#h
...
get_${BL_METHOD}
#old is now what is the current setting
$BL_NO_SET && echo "$((( 100 * $OLD ) / $MAX ))"
